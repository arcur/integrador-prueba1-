  ...
  try {
    ...
    for (const item of cart.items) {
      ...
      if (rows[0].stock < item.cantidad) {
        ...
        throw new Error(
          `Stock insuficiente para: ${item.nombre_producto}. Stock disponible: ${rows[0].stock}`
        );
      }
    }
    ...
    for (const item of cart.items) {
      ...
      for (const lote of lotes) {
        ...
        const cantidad_a_consumir = Math.min(
          cantidad_necesaria,
...
Pedido.getDetalleById = async (id_pedido) => {
  ...
  const sqlPedido = `
    SELECT 
        ...
        p.id_usuario  /* <-- ¡¡ESTA LÍNEA FALTABA!! */
    FROM pedido p
    LEFT JOIN usuario u ON p.id_usuario = u.id_usuario
    WHERE p.id_pedido = ?
`;
...
Pedido.updateStatus = async (id_pedido, nuevo_estado) => {
  ...
  try {
    ...
    const [result] = await connection.query(sqlUpdatePedido, [
      nuevo_estado,
      id_pedido,
    ]);
